...
%----------------------------updateRelPerm---------------------------------
function model = setMyRelpermFun(model, varargin)

% set parameter value

model = setfield(model, varargin{:});

if isa(model.fluid.kW,'ADI')
    kW=model.fluid.kW.val;
else
    kW=model.fluid.kW;
end

if isa(model.fluid.kG,'ADI')
    kG=model.fluid.kG.val;
else
    kG=model.fluid.kG;
end

if isa(model.fluid.nW,'ADI')
    nW=model.fluid.nW.val;
else
    nW=model.fluid.nW;
end

if isa(model.fluid.nG,'ADI')
    nG=model.fluid.nG.val;
else
    nG=model.fluid.nG;
end

if isa(model.fluid.srW,'ADI')
    srW=model.fluid.srW.val;
else
    srW=model.fluid.srW;
end

if isa(model.fluid.srG,'ADI')
    srG=model.fluid.srG.val;
else
    srG=model.fluid.srG;
end
% update function handles
number_of_regions=4; %need to update auto

for i=1:number_of_regions

%     model.fluid.krG{i}=coreyPhaseRelpermAD(model.fluid.nW(i), model.fluid.srW(i), model.fluid.kW(i), model.fluid.srW(i));
%     model.fluid.krW{i}=coreyPhaseRelpermAD(model.fluid.nG(i), model.fluid.srG(i), model.fluid.kG(i), model.fluid.srW(i));

    model.fluid.krG{i}=coreyPhaseRelpermAD(nW(i), srW(i), kW(i), srW(i));
    model.fluid.krW{i}=coreyPhaseRelpermAD(nG(i), srG(i), kG(i), srW(i));
